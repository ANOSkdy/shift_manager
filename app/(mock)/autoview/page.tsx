...
type AutoShiftRow = {
  ...
  startTime: string;
  endTime: string;
  allocationStatus: '希望' | '仮割当' | '要調整';
  priority: '高' | '中' | '低';
};

const autoShiftRows: AutoShiftRow[] = [
    ...
    <div className="page-stack">
          ...
          <table>
            ...
            <tbody>
              {autoShiftRows.map((row) => (
                <tr key={row.id}>
                  <td>{row.workDate}（{row.weekday}）</td>
                  <td>{row.shiftType}<br /><span className="muted">{row.startTime} - {row.endTime}</span></td>
                  <td>{row.employeeName}</td>
                  <td>{row.team}</td>
                  <td>{row.assignedSite}</td>
                  <td>{row.taskType}</td>
                  <td><StatusTag tone={row.priority === '高' ? 'danger' : row.priority === '中' ? 'warning' : 'insight'}>{row.priority}</StatusTag></td>
                  <td>
      ...
      <SectionCard title="次のアクション">
        <div className="status-row">
          <Link href="/adjustment" className="primary-button">調整画面へ</Link>
          <Link href="/pre-final" className="chip">確定前チェックへ</Link>
        </div>
      </SectionCard>
    </div>
  );
